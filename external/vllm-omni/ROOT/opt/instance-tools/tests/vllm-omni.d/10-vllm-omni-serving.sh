...
# bakes no port (vllm-omni.sh interpolates VLLM_ARGS verbatim), so a template that moves
...
VLLM_LOG="/var/log/vllm-omni.log"
...
if [[ -f /etc/supervisor/conf.d/ray.conf ]]; then
    ...
    # Verify gcs_server process is alive (vllm-omni.sh waits for this)
...
assert_service_running vllm-omni
echo "  vllm-omni: supervisor service running"

# Check for early fatal errors in the log before waiting for the API.
# vLLM-Omni logs to /var/log/vllm-omni.log (clean, ANSI-stripped).
if [[ -f "$VLLM_LOG" ]]; then
...
last_pid=$(supervisorctl status vllm-omni 2>/dev/null | grep -oP 'pid \K[0-9]+' || true)

_dump_log_tail() {
    if [[ -f "$VLLM_LOG" ]]; then
        echo "  last 10 lines of vllm-omni log:"
...
while (( elapsed < HEALTH_TIMEOUT )); do
    ...
    sup_line=$(supervisorctl status vllm-omni 2>/dev/null)
    vllm_sup_state=$(echo "$sup_line" | awk '{print $2}')
    cur_pid=$(echo "$sup_line" | grep -oP 'pid \K[0-9]+' || true)

    if [[ "$vllm_sup_state" == "FATAL" ]]; then
        echo "  vllm-omni entered FATAL state — supervisor gave up restarting"
        _dump_log_tail
        test_fail "vllm-omni service FATAL (supervisor stopped restarting)"
    fi

    if [[ "$vllm_sup_state" != "RUNNING" && "$vllm_sup_state" != "STARTING" ]]; then
        echo "  vllm-omni service in state: ${vllm_sup_state}"
        _dump_log_tail
        test_fail "vllm-omni service not running (state: ${vllm_sup_state})"
    fi

    # Detect restart: PID changed while we were waiting
    if [[ -n "$last_pid" && -n "$cur_pid" && "$cur_pid" != "$last_pid" ]]; then
        restart_count=$((restart_count + 1))
        echo "  vllm-omni restarted (pid ${last_pid} → ${cur_pid}, restart #${restart_count})"
        _dump_log_tail
        if (( restart_count >= MAX_RESTARTS )); then
            test_fail "vllm-omni crash loop: ${restart_count} restarts detected (bad config or OOM?)"
        fi
    fi
    last_pid="$cur_pid"
...
if ! $healthy; then
    ...
    if [[ -f "$VLLM_LOG" ]]; then
        echo "  last 15 lines of vllm-omni log:"
...
declare -A PORT_CHECKS=(
    ["vllm-omni"]="8000:vLLM-Omni API:vllm-omni"
