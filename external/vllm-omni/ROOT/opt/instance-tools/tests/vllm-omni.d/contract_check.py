...
repo (CLAUDE.md); `10-vllm-omni-serving.sh` already reaches for a heredoc `python3 -c`
...
they can be unit-tested off-box (tools/imagegen/tests/test_vllm_contract_check.py, which covers the vllm copy)
...
model. Specifically refused, and these refusals are binding:
  ...
  - content matching ("2+2" -> "4"): that is model competence, and the model is
    chosen per template. `10-vllm-omni-serving.sh` already learned this the expensive
