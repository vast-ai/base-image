...
#
# 10-vllm-omni-serving.sh answers "did the engine come up and emit a token". This answers
...
# claims about. 10-vllm-omni-serving is the file the gate REQUIRES, so an image that lost
...
# 10-vllm-omni-serving has already waited out model load, but it must also hold when run
...
wait_for_url "${VLLM_API}/health" "$CONTRACT_READY_TIMEOUT" \
    || test_fail "vLLM-Omni /health not reachable on :${VLLM_INTERNAL_PORT} after ${CONTRACT_READY_TIMEOUT}s"
echo "  vLLM-Omni healthy on :${VLLM_INTERNAL_PORT}"

# The checker's chat-template round trip needs the interpreter that has transformers,
# which is the engine venv — /venv/main is what vllm-omni.sh activates before `vllm serve`.
...
# `sock_addr = (args.host or "", args.port)`) and vllm-omni.sh injects no host, so a
...
report_failures
if [[ "${enforce,,}" == "true" ]]; then
    test_pass "vLLM-Omni API contract verified — enforcing, ${violations} violation(s)"
fi
test_pass "vLLM-Omni API contract checks complete — ${violations} violation(s), advisory (VLLM_CONTRACT_ENFORCE=false)"
