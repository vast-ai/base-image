...
VLLM_EXPECT_CAPS but not discovered is a VIOLATION (claimed and absent); one that is
...
def check_context_overflow(rep: Report, cli: Client, model: str, models_doc, tokens: list[str]) -> None:
    ...
    for m in (models_doc or {}).get("data", []):
        if m.get("id") == model and isinstance(m.get("max_model_len"), int):
            ...
            break
    if length is None:
        declared = first(arg_values(tokens, "--max-model-len"))
        if declared.isdigit():
            length = int(declared)
    if length is None:
...
def cap_tools(cli: Client, model: str, tokens: list[str]):
    ...
    if not arg_present(tokens, "--enable-auto-tool-choice"):
        return None, "--enable-auto-tool-choice not in VLLM_ARGS"
...
def run_capabilities(rep: Report, cli: Client, model: str, tokens: list[str],
    ...
    if unknown:
        rep.error("capabilities",
                  f"VLLM_EXPECT_CAPS names unknown capabilities {sorted(unknown)} — "
        ...
        if verdict is None:                       # not discovered on this box
            if name in declared:
                rep.violation(f"cap:{name}",
                              f"declared in VLLM_EXPECT_CAPS but not discovered — {detail}")
            else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--vllm-args", default=os.environ.get("VLLM_ARGS", ""))
    p.add_argument("--expect-caps", default=os.environ.get("VLLM_EXPECT_CAPS", ""),
