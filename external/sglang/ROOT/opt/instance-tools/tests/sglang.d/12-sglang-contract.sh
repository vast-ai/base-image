...
#
# 10-sglang-serving.sh answers "did the engine come up and emit a token". This answers
...
#
# ENFORCING BY DEFAULT, with `SGLANG_CONTRACT_ENFORCE=false` as the escape hatch —
...
# claims about. 10-sglang-serving is the file the gate REQUIRES, so an image that lost
# SGLANG_MODEL is already red there (L057/L072) rather than quietly green here.
[[ -n "${SGLANG_MODEL:-}" ]] || test_skip "SGLANG_MODEL not set — no served surface to assert"
...
# 10-sglang-serving has already waited out model load, but it must also hold when run
# alone from an SSH session on a box that has just booted.
CONTRACT_READY_TIMEOUT="${SGLANG_CONTRACT_READY_TIMEOUT:-1200}"

echo ""
echo "  -- readiness --"
wait_for_url "${SGLANG_API}/health" "$CONTRACT_READY_TIMEOUT" \
    || test_fail "SGLang /health not reachable on :${SGLANG_INTERNAL_PORT} after ${CONTRACT_READY_TIMEOUT}s"
echo "  SGLang healthy on :${SGLANG_INTERNAL_PORT}"

# The checker's chat-template round trip needs the interpreter that has transformers,
# which is SGLang's own venv — /venv/main is what sglang.sh activates before `sglang serve`.
...
# customer. The assertion most likely to want it is bind-loopback: `sglang serve`
# binds ALL interfaces when --host is absent (api_server.py:
# `sock_addr = (args.host or "", args.port)`). SGLang is the safer of the two — its
# ServerArgs dataclass defaults host to 127.0.0.1 — but the template still states the
# bind explicitly rather than inheriting an upstream default that can change, and a
# template that sets --host 0.0.0.0 fails here, correctly, since that cell would be
# serving the engine past the Caddy auth gate.
enforce="${SGLANG_CONTRACT_ENFORCE:-true}"
report=$(mktemp)
...
# single-token SGLANG_ARGS — `--enforce-eager`, which is a legitimate value — is read
...
"$PY" "$CHECKER" \
    ...
    --model="$SGLANG_MODEL" \
    --engine-args="${SGLANG_ARGS:-}" \
    --expect-caps="${SGLANG_EXPECT_CAPS:-}" \
    > "$report" 2>&1
rc=$?
...
if (( violations > 0 )); then
    if [[ "${enforce,,}" == "true" ]]; then
        ...
        echo "  (SGLANG_CONTRACT_ENFORCE=false — ADR 0031 decision 7: two consecutive green"
...
report_failures
if [[ "${enforce,,}" == "true" ]]; then
    test_pass "SGLang API contract verified — enforcing, ${violations} violation(s)"
fi
test_pass "SGLang API contract checks complete — ${violations} violation(s), advisory (SGLANG_CONTRACT_ENFORCE=false)"
