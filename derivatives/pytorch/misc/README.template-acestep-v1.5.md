...
This template gives you a **complete AI music generation environment** with ACE-Step 1.5 running on a Vast.ai PyTorch instance. ACE-Step provides a state-of-the-art music generation model with both an API server and a dedicated web UI for creating music from text prompts.

**Think:** *"AI music generation made easy - describe a song and let ACE-Step compose it for you!"*

> **Provisioning-based template:** This template runs on the standard `vastai/pytorch` image. On first boot, a provisioning script automatically installs ACE-Step 1.5 and its web UI into your workspace. Initial setup takes a few minutes as dependencies are downloaded and installed.
...
- **`ACE_STEP_REF`**: Git ref for ACE-Step 1.5 (default: `main`)
- **`ACE_STEP_UI_REF`**: Git ref for the ACE-Step UI (default: `main`)
...
This template is provisioning-based - it uses the `PROVISIONING_SCRIPT` environment variable to install ACE-Step 1.5 on first boot. You can replace it with your own script URL to customize the installation.
...
| `ACE_STEP_REF` | `main` | Git ref (branch/tag/commit) for ACE-Step 1.5 |
| `ACE_STEP_UI_REF` | `main` | Git ref (branch/tag/commit) for ACE-Step UI |
| `ACESTEP_LM_MODEL_PATH` | `acestep-5Hz-lm-4B` | Language model path for generation |
| `PROVISIONING_SCRIPT` | (set by template) | URL to the provisioning script |
...
This template runs on the standard `vastai/pytorch` images, which are tagged with the CUDA version they were built against. This does not mean you need that exact CUDA version on the host.
