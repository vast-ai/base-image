...
build_image() {
    ...
    if [[ -z "$torch_ver" ]] || [[ -z "$python_ver" ]] || [[ -z "$cuda_ver" ]] || [[ -z "$torch_backend" ]]; then
        ...
        echo "Usage: build-many-24.04.sh --torch_ver=<ver> --python_ver=<ver> --cuda_ver=<ver> --torch_backend=<backend> [--multi_arch]"
        return 1
    fi

    # Set up tags based on python version
    tags="--tag ${TAG_REPO}:${torch_ver_override:-$torch_ver}-cuda-${cuda_ver%%-*}-py${python_ver}-24.04"
    if [[ "$python_ver" == "312" ]]; then
        tags+=" --tag ${TAG_REPO}:${torch_ver_override:-$torch_ver}-cuda-${cuda_ver%%-*}-24.04"
    fi
