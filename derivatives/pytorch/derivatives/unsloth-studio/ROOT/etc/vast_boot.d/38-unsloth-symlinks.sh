...
# when the target is ABSENT, so an instance started against a volume that already holds
# a $WORKSPACE/unsloth from an older image would otherwise keep running that image's
# llama.cpp — including a build predating the CUDA fix — no matter what this image
# contains, and LLAMA_CPP_RELEASE would then describe the wrong binary. Repoint the
# studio's install path at the copy baked into THIS image on every boot.
#
# Only the build tree is replaced. Datasets, runs and exports elsewhere under
# $WORKSPACE/unsloth are untouched.
