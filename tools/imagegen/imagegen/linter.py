...
RULES: list[tuple[str, str, str]] = [
    ...
    ("L097", ERROR, "Every test a WORKFLOW requires -- in a qa-gate caller's `require_tests`, or in the `INSTANCE_TEST_REQUIRE_PASS` it injects through `extra_env` -- names a test file that exists in the tree: `base/<n>` under the base image's own suite, `<engine>.d/<n>` under that image's overlay. L057, L059 and L072 read the TEMPLATE's `env.INSTANCE_TEST_REQUIRE_PASS` and cannot see these: a serverless cell declares its required set in the workflow, because the template is shared with the on-demand cell that must not require serverless tests. So the one place the serverless requirements are written was the one place nothing checked them. A name with no file cannot pass, so the gate fails -- but it fails on a RENTED GPU after a full image build, reporting a missing test rather than a missing FILE, and the fact is visible in the repo the whole time (ADR 0001: static checks are the fast gate). Same shape as L088 one level up: there a test reached for a sibling helper the image did not ship, and the discovery cost a live cell. Found while wiring vllm-omni's serverless cell (ADR 0044) -- deleting `vllm-omni.d/20-serverless-pyworker.sh` left the workflow requiring it and every static check clean"),
...
_WF_REQUIRE = re.compile(r"require_tests:\s*[\"']([^\"']+)[\"']")
_WF_REQUIRE_ENV = re.compile(r"INSTANCE_TEST_REQUIRE_PASS=([^\n]+)")
_TEST_NAME = re.compile(r"\b((?:base|[a-z0-9.-]+\.d)/[0-9]+-[a-z0-9-]+)\b")


def check_workflow_required_tests_exist(repo: Path) -> Iterable[Finding]:
    """L097 — every test a workflow requires is a file in the tree."""
    suites: dict[str, Path] = {}
    for tests_dir in sorted(repo.glob("**/ROOT*/opt/instance-tools/tests")):
        for suite in sorted(tests_dir.iterdir()):
            if suite.is_dir():
                suites.setdefault(suite.name, suite)
    ...
    for wf in sorted(repo.glob(".github/workflows/*.yml")):
        ...
        for pat in (_WF_REQUIRE, _WF_REQUIRE_ENV):
            for m in pat.finditer(text):
                names.update(_TEST_NAME.findall(m.group(1)))
        for name in sorted(names):
            suite_name, _, stem = name.partition("/")
            suite = suites.get(suite_name)
            if suite is None:
                yield Finding("L097", ERROR, "-", f".github/workflows/{wf.name}",
                              f"requires `{name}`, but no `{suite_name}/` test suite "
                              f"exists anywhere in the tree (L097)")
                continue
