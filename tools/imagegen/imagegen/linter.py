...
RULES: list[tuple[str, str, str]] = [
    ...
    ("L073", ERROR, "A QA template booted by a gate that sets SERVERLESS=true MAPS the serverless worker port (WORKER_PORT, default 3000), and does not rely on the image's EXPOSE to do it. HISTORY CORRECTED 2026-09-22: this rule's original text said the platform injects VAST_TCP_PORT_<n> ONLY for ports a template maps. That is FALSE -- an EXPOSEd port is mapped too. The measurement behind the original claim is real but predates the mechanism it was used to deny: the KeyError was recorded at 12:30 on 2026-08-24 (0b31803) and `EXPOSE 3000` first entered the engine images at 18:29 the same day (2fea0c9), so at the moment of that KeyError there was nothing exposed to map. What the SDK does is unchanged and is why the port must come from somewhere: `worker_port = os.environ[f\"VAST_TCP_PORT_{os.environ['WORKER_PORT']}\"]` is an unguarded lookup, so a missing mapping is a KeyError in Metrics() during Backend construction, before the worker binds a port or runs a benchmark. The rule stands on a narrower and truer footing: the template must not depend on the IMAGE continuing to EXPOSE the port, since an image can drop an EXPOSE without any template noticing, and an explicit entry is visible to whoever reads the template. Harmless on the standard cells: nothing binds 3000 there, and base/28 scans LISTENERS rather than mappings, so an unused mapping is not an exposure. Vast's port declarations exist so many instances can share one address, not as a security boundary"),
        ...
        for entry in (data if isinstance(data, list) else [data]):
            ...
            if port not in mapped:
                yield Finding("L073", ERROR, img.name, rel,
                              ...
                              f"the pyworker SDK looks VAST_TCP_PORT_{port} up unguarded, so the worker "
                              "dies with a KeyError inside Metrics() before it binds or benchmarks. An "
                              "EXPOSE in the image also maps it, but the template must not DEPEND on "
                              "that: an image can drop an EXPOSE without any template noticing")
...
def check_llama_cuda_assert(img: Image) -> Iterable[Finding]:
    ...
    # `ldd -r` EXITS 0 while printing `undefined symbol`, so running it is not the check
    # — reading its output is. Require both the invocation on the artifact and a match on
    # the failure strings, or the guard is decoration.
    if not re.search(r"\bldd\s+-r\s+\S*libggml-cuda\.so|\bldd\s+-r\s+\"?\$\{?LLAMA_CUDA", code):
        yield Finding("L056", ERROR, img.name, "Dockerfile",
                      "installs Unsloth Studio's llama.cpp but never runs `ldd -r` on libggml-cuda.so — "
                      "with a PREBUILT bundle `test -f` is satisfied by `tar x`, and GGML_BACKEND_DL=ON "
                      "means an unresolvable backend is skipped silently and every inference runs on "
                      "CPU (ADR 0016, ADR 0018)")
    elif not re.search(r"not found\|undefined symbol|undefined symbol\|not found", code):
        yield Finding("L056", ERROR, img.name, "Dockerfile",
                      "runs `ldd -r` on libggml-cuda.so but never inspects the output for `not found` / "
                      "`undefined symbol` — ldd -r exits 0 while reporting them, so an unchecked run "
                      "asserts nothing (ADR 0018)")


def check_llama_sass_coverage(img: Image) -> Iterable[Finding]:
    """L081 — the shipped CUDA backend must carry SASS for the admitted GPU range."""
    code = code_text(parse(img.text))
    if not re.search(r"\bunsloth\s+studio\s+setup\b", code):
        return
    if not re.search(r"cuobjdump\s+--list-elf\s+\S*libggml-cuda\.so|cuobjdump\s+--list-elf\s+\"?\$\{?LLAMA_CUDA", code):
        yield Finding("L081", ERROR, img.name, "Dockerfile",
                      "installs Unsloth Studio's llama.cpp but never checks the SASS arch coverage of "
                      "libggml-cuda.so — an admitted GPU with no kernel image CRASHES (no-kernel-image), "
                      "it does not fall back, so neither existence nor `ldd -r` can see it "
                      "(`cuobjdump --list-elf`) (ADR 0018)")
        return
    if len(set(re.findall(r"\bsm_(\d+)\b", code))) < 2:
        yield Finding("L081", ERROR, img.name, "Dockerfile",
                      "checks SASS coverage against fewer than two literal sm_NN targets — a build can "
                      "satisfy one end of the admitted range and miss the other, so name the BRACKET "
                      "(the template's compute_cap floor and its ceiling) (ADR 0018)")
...
# L091 - AI Toolkit's UI binds every interface unless pinned. Detect the image by the
# upstream repo name, which both images clone.
_AITK_MARK = re.compile(r"ai-toolkit")


def check_ai_toolkit_binds_loopback(img: Image) -> Iterable[Finding]:
    """L091 - an image that installs AI Toolkit pins its public UI listener to loopback.

    Two obligations. The bind must actually be pinned to 127.0.0.1, and the patch must
    FAIL the build when it does not apply - BOTH images resolve the upstream ref in CI,
    so neither is anchored to a known launch shape and a patch that silently matched
    nothing would ship a public bind."""
    code = code_text(parse(img.text))
    if not _AITK_MARK.search(code):
        return
    # The pin must be attached to AI TOOLKIT's own launch, not merely present. A
    # whole-file check is useless here: aio-studio builds nine apps and already pins
    # loopback for ComfyUI (`--listen 127.0.0.1`) and voicebox (`--host 127.0.0.1`),
    # either of which satisfied the first draft while AI Toolkit stayed wide open.
    # Naming the upstream launch shapes is deliberate — when they change, the FATAL
    # obligation below turns that into a failed build rather than a silent pass.
    _PINNED = (r"(fileServer|next start|publicPort|--hostname)[^\n]*127\.0\.0\.1"
               r"|127\.0\.0\.1[^\n]*(fileServer|next start|publicPort|--hostname)")
    if not re.search(_PINNED, code):
        yield Finding("L091", ERROR, img.name, "Dockerfile",
                      "installs AI Toolkit but never pins its UI listener to loopback - "
                      "upstream's public server takes no host argument and binds 0.0.0.0, "
                      "reachable however the platform maps ports; everything behind Caddy "
                      "binds 127.0.0.1")
    elif not re.search(r"FATAL[^\n]*(bind|loopback|listen|127\.0\.0\.1)", code, re.I):
        yield Finding("L091", ERROR, img.name, "Dockerfile",
                      "pins AI Toolkit's bind but does not fail the build when the patch "
                      "does not apply - one image floats to the latest upstream commit, so "
                      "a launch shape that stops matching would silently restore the public "
                      "bind; an unrecognised shape must be FATAL")


# L092 - a curl that writes a file must use --fail, or an HTTP error body becomes the
# artifact. Blank quoted strings and bare URLs BEFORE looking for flags: `-f` occurs
# inside real URLs (conda-forge), and matching it there is a false pass.
_CURL_WRITES = re.compile(r"\bcurl\b[^\n|;&]*")
_URLISH = re.compile(r"\"[^\"]*\"|'[^']*'|https?://\S+")


def check_curl_download_fails_on_http_error(img: Image) -> Iterable[Finding]:
    """L092 - `curl -o` carries `--fail`, so a 404/504 body never becomes the artifact."""
    # Read the RAW Dockerfile, not code_text, and join backslash continuations before
    # scanning. A hardened curl is usually wrapped across lines, putting `-o` on a
    # different line from `curl`; a scan that stops at the newline sees no `-o`, skips
    # the command as "not writing a file", and silently exempts the exact shape this
    # rule exists to police. code_text cannot be used for the join because it strips the
    # backslashes and keeps the newlines, leaving nothing to join on — so drop comment
    # lines here instead, which is the only thing code_text was wanted for.
    # The mutation test is what caught this; the first draft was decorative.
    _raw = "\n".join(l for l in img.text.splitlines() if not l.lstrip().startswith("#"))
    code = re.sub(r"\\\s*\n\s*", " ", _raw)
    for m in _CURL_WRITES.finditer(code):
        cmd = _URLISH.sub(" URL ", m.group(0))
        if not re.search(r"(?:^|\s)-[A-Za-z]*[oO](?:\s|$)", cmd):
            continue  # not writing a file; whatever consumes it can judge it
        if re.search(r"(?:^|\s)-[A-Za-z]*f[A-Za-z]*(?:\s|$)|--fail", cmd):
            continue
        yield Finding("L092", ERROR, img.name, "Dockerfile",
                      f"`{m.group(0).strip()[:60]}...` writes a file without --fail - curl "
                      f"saves the HTTP ERROR BODY and exits 0, so a 504 page becomes the "
                      f"artifact and the failure surfaces later as nonsense (a 504 HTML page "
                      f"executed as a shell script)")
...
# RFC 2606 reserves `.invalid` precisely so it can never resolve; a loopback literal
# cannot leave the box either. Anything else — including an empty value — is treated as
# reaching the outside world, because the DEFAULT is production (L079).
_UNREACHABLE_REPORT_ADDR = re.compile(
    r"^\w+://(?:[^/@]*@)?(?:[^/:]+\.invalid|127\.0\.0\.1|\[::1\]|localhost)(?::\d+)?(?:/|$)", re.I)


def check_serverless_gate_cannot_reach_production(repo: Path) -> Iterable[Finding]:
    """L079 — a serverless QA cell must not be able to POST to the real autoscaler."""
    wfdir = repo / ".github" / "workflows"
    if not wfdir.is_dir():
        return
    import yaml  # lazy
    for wf in sorted(wfdir.iterdir()):
        ...
        for jobname, job in (data.get("jobs") or {}).items():
            if not isinstance(job, dict) or not str(job.get("uses", "")).endswith("qa-gate.yml"):
                continue
            env = str((job.get("with") or {}).get("extra_env", "") or "")
            pairs = dict(
                ln.split("=", 1) for ln in env.splitlines()
                if "=" in ln and not ln.lstrip().startswith("#"))
            declared = pairs.get("SERVERLESS", "").strip().lower() == "true"
            inferred = bool(pairs.get("MASTER_TOKEN", "").strip()
                            and pairs.get("REPORT_ADDR", "").strip())
            if not (declared or inferred):
                continue                   # the worker never starts; nothing to post
            addr = pairs.get("REPORT_ADDR", "").strip()
            if not addr:
                yield Finding("L079", ERROR, "-", f".github/workflows/{wf.name}",
                              f"job `{jobname}` enables serverless but sets no REPORT_ADDR — "
                              f"unset does not mean nowhere, it means "
                              f"https://run.vast.ai (start_server.sh and the SDK both default "
                              f"to it), so this cell posts worker status to the PRODUCTION "
                              f"autoscaler from a disposable QA instance")
            elif not _UNREACHABLE_REPORT_ADDR.match(addr):
                yield Finding("L079", ERROR, "-", f".github/workflows/{wf.name}",
                              f"job `{jobname}` points REPORT_ADDR at {addr!r}, which is not a "
                              f"provably unreachable address — use a host under the RFC 2606 "
                              f"`.invalid` TLD or a loopback literal, so the cell cannot reach "
                              f"any live endpoint whatever it is renamed to")


# UNSECURED disables the pubkey gate AND per-request signature verification
# (__check_signature returns True immediately). Legitimate in a QA gate's extra_env,
# never in a template a customer could launch or copy from (L080).
_UNSECURED_DECL = re.compile(r"(?m)^\s*(?:-\s*)?[\"']?UNSECURED[\"']?\s*[:=]")


def check_no_template_declares_unsecured(repo: Path) -> Iterable[Finding]:
    """L080 — UNSECURED is QA scaffolding; it must not live in a template."""
    for tpl in sorted(repo.rglob("templates/**/template.yml")):
        if ".git" in tpl.parts:
            continue
        try:
        ...
        for lineno, raw in enumerate(text.splitlines(), 1):
            line = raw.split("#", 1)[0]
            if _UNSECURED_DECL.match(line):
                yield Finding("L080", ERROR, "-", str(tpl.relative_to(repo)) + f":{lineno}",
                              "template declares UNSECURED — it disables the pubkey gate and makes "
                              "__check_signature accept every request unverified, so a live worker "
                              "launched from this template would serve unauthenticated. It is QA "
                              "scaffolding and belongs in the gate's extra_env, which a copied "
...
# L087 — a CUDA label for an upstream image must come from the artifact (ADR 0035).
# Matches a literal like `"cuda":"13.0"` or `cuda: "12.9"` being assigned as a build
# matrix value. The evidence that a step did the right thing instead is that it reads
# CUDA_VERSION out of the image config.
_CUDA_LITERAL = re.compile(r'["\']?cuda["\']?\s*:\s*["\'][0-9]+\.[0-9]+["\']')


def check_upstream_cuda_label_comes_from_the_artifact(repo: Path) -> Iterable[Finding]:
    """L087 — an upstream image's CUDA label is read, not inferred from its tag name."""
    wfdir = repo / ".github" / "workflows"
    if not wfdir.is_dir():
        return
    import yaml  # lazy
    for wf in sorted(wfdir.iterdir()):
        if wf.suffix not in (".yml", ".yaml") or not wf.is_file():
            continue
        text = wf.read_text(encoding="utf-8", errors="replace")
        # Scope: only workflows that resolve SOMEONE ELSE'S image by tag. A matrix
        # naming our own pytorch base is a build input, not a claim about a foreign
        # artifact, and must not be caught here.
        if "check-dockerhub-release" not in text:
            continue
        try:
        ...
        for jobname, job in (data.get("jobs") or {}).items():
            ...
            for step in (job.get("steps") or []):
                ...
                if not run or not _CUDA_LITERAL.search(run):
                    continue
                # PER STEP: a sibling step reading the artifact does not excuse this
                # one. That is exactly how build-vllm-omni.yml kept a hardcoded
                # nightly label after its release path was converted.
                if "CUDA_VERSION" in run:
                    continue
                name = step.get("name") or step.get("id") or "<unnamed>"
                yield Finding("L087", ERROR, "-", f".github/workflows/{wf.name}",
                              f"job `{jobname}` step `{name}` assigns a literal CUDA version to "
                              f"an upstream image's build matrix — read it from the artifact "
                              f"(`docker buildx imagetools inspect --format '{{{{ json .Image }}}}'` "
                              f"then CUDA_VERSION) and fail rather than guess, because upstream "
                              f"re-points tags without renaming them (ADR 0035)")



# L088 - a test that sources a sibling helper must ship it. `$(dirname "$0")/NAME`
# where NAME is a bare filename; `/../lib.sh` and friends are path segments, not
# siblings, and are deliberately not matched.
_SIBLING_REF = re.compile(r'\$\(dirname\s+"\$0"\)/([A-Za-z0-9_][A-Za-z0-9_.-]*)')


def check_a_test_ships_the_sibling_it_reaches_for(repo: Path) -> Iterable[Finding]:
    """L088 - a referenced sibling helper exists beside the test that needs it."""
    for sh in sorted(repo.rglob("opt/instance-tools/tests/**/*.sh")):
        if not sh.is_file():
            continue
        try:
            ...
            continue
        for name in dict.fromkeys(_SIBLING_REF.findall(text)):
            if (sh.parent / name).exists():
                continue
            yield Finding("L088", ERROR, "-", str(sh.relative_to(repo)),
                          f"reaches for `{name}` beside it but that file is not in "
                          f"{sh.parent.relative_to(repo)} - a suite copied file-by-file "
                          f"instead of directory-by-directory ships without its helper, "
                          f"and the gap only surfaces on a rented GPU")


# L094 - a declared in-image copyleft licence path must resolve.
_LIC_ENTRY = re.compile(r"^## (?P<app>.+?)$(?P<body>.*?)(?=^## |\Z)", re.M | re.S)
_LIC_KIND = re.compile(r"\*\*License:\*\*\s*(?P<lic>[^\n]+)")
_LIC_PATH = re.compile(r"\*\*License file in image:\*\*\s*`(?P<p>[^`]+)`")
_LIC_COPYLEFT = re.compile(r"\bAGPL\b|\bGPL-[23]", re.I)
# Paths that only exist after a build: an upstream clone, or a versioned install dir.
# Checking these statically would be guessing, so they are declared out of scope rather
# than silently passed.
_LIC_RUNTIME = ("$WORKSPACE", "/opt/workspace-internal", "/opt/blender", "/opt/voicebox")


def check_copyleft_licence_path_resolves(repo: Path) -> Iterable[Finding]:
    """L094 - a copyleft entry declares a licence path, and an image-provided one exists."""
    for lm in sorted(repo.rglob("*/LICENSES.md")):
        if "/pcl/" in str(lm):
            continue
        root = lm.parent                      # the ROOT* overlay the file sits in
        try:
            text = lm.read_text(encoding="utf-8", errors="replace")
        except Exception:
            continue
        rel = lm.relative_to(repo)
        for m in _LIC_ENTRY.finditer(text):
            body, app = m.group("body"), m.group("app").strip()
            kind = _LIC_KIND.search(body)
            if not kind or not _LIC_COPYLEFT.search(kind.group("lic")):
                continue
            pm = _LIC_PATH.search(body)
            if not pm:
                yield Finding("L094", ERROR, "-", str(rel),
                              f"`{app}` is declared copyleft but names no "
                              f"`**License file in image:**` - GPL/AGPL require the licence "
                              f"text to accompany the program, and nothing here says where "
                              f"it ships")
                continue
            path = pm.group("p").strip()
            if path.startswith(_LIC_RUNTIME):
                continue                      # only exists after a build; not our claim to check
            cand = root / path.lstrip("/")
            if not cand.exists():
                yield Finding("L094", ERROR, "-", str(rel),
                              f"`{app}` declares its licence ships at `{path}`, but "
                              f"{cand.relative_to(repo)} is not in the overlay - the image "
...
# expression. YAML folds it in and the workflow stops parsing entirely.
def _blank_quoted(expr: str) -> str:
    """Blank single/double-quoted spans so a legal `'#skip'` cannot look like a comment."""
    out, quote = [], None
    for ch in expr:
        if quote:
            out.append(" ")
...
def _walk_if_values(node):
    """Yield every value of an `if:` key anywhere in the workflow tree."""
    if isinstance(node, dict):
        for k, v in node.items():
            if k == "if" and isinstance(v, str):
                yield v
            yield from _walk_if_values(v)
    elif isinstance(node, list):
        for v in node:
            yield from _walk_if_values(v)


def check_workflow_if_has_no_folded_comment(repo: Path) -> Iterable[Finding]:
    """L095 — a workflow `if:` block scalar must not carry a `#` comment line."""
    wfdir = repo / ".github" / "workflows"
    if not wfdir.is_dir():
        return
    import yaml  # lazy
    for wf in sorted(wfdir.iterdir()):
        ...
        try:
            ...
            continue                       # malformed YAML is a different problem
        for expr in _walk_if_values(data):
            if "#" not in _blank_quoted(expr):
                continue
            frag = expr[max(0, _blank_quoted(expr).index("#") - 40):][:110]
            yield Finding("L095", ERROR, "-", str(wf.relative_to(repo)),
                          "a `#` comment inside this block-scalar `if:` is folded INTO "
                          "the expression, so the whole workflow fails to parse before "
                          "any job is created (no step log, no Slack alert, "
                          "workflow_dispatch refused with HTTP 422). Move the prose "
                          f"above the `if:` key. Near: ...{frag.strip()}... (L095)")



# L096 — a vLLM-derived image must install the engine's audio extra and prove it imports.
_VLLM_BASE_ARG = re.compile(r"^ARG\s+\w*VLLM\w*BASE\s*=\s*(vllm/vllm-[\w.-]+)", re.M)
_AUDIO_INSTALLER = "tools/install-vllm-audio-extras.sh"


def check_vllm_audio_extra_is_installed(repo: Path) -> Iterable[Finding]:
    """L096 — vLLM's audio extra is installed from metadata, and imported to prove it."""
    for df in sorted(repo.glob("external/*/Dockerfile")):
        try:
            text = df.read_text(encoding="utf-8", errors="replace")
        except Exception:
            continue
        if not _VLLM_BASE_ARG.search(text):
            continue                       # not a vLLM-derived image; the extra is vLLM's
        rel = df.relative_to(repo)
        if _AUDIO_INSTALLER not in text:
            yield Finding("L096", ERROR, "-", str(rel),
                          f"builds FROM a vLLM upstream image but never runs "
                          f"{_AUDIO_INSTALLER} - /v1/audio/transcriptions and "
                          f"/v1/audio/translations are registered by the engine and would "
                          f"answer 400 for every request (L096)")
            continue
        script = repo / _AUDIO_INSTALLER
        if not script.exists():
            yield Finding("L096", ERROR, "-", str(rel),
                          f"runs {_AUDIO_INSTALLER}, which is not in the tree (L096)")
            continue
        body = script.read_text(encoding="utf-8", errors="replace")
        if "Requires-Dist" not in body:
            yield Finding("L096", ERROR, "-", _AUDIO_INSTALLER,
                          "does not read the extra from vLLM's own metadata "
                          "(`Requires-Dist`) - a hardcoded package list is wrong for some "
                          "engine tag this same Dockerfile builds (L096)")
        if "import_module" not in body:
            yield Finding("L096", ERROR, "-", _AUDIO_INSTALLER,
                          "installs the extra without importing it - installing is not "
                          "resolving, and an unimportable wheel leaves the audio routes "
...
#
# Both patterns are matched against WHOLE TOKENS, split the way the consumers split them
# (runner.sh: `${INSTANCE_TEST_REQUIRE_PASS//,/ }` then `read -ra`; qa_verdict.py's
# parse_required: `replace(",", " ").split()`). Searching the value for matches instead
# let everything that matched NEITHER pattern vanish, and let a partial match stand in
# for the token: `foo-base/60-gpu-cuda` was read as `base/60-gpu-cuda` and passed.
_TEST_NAME = re.compile(r"(?:base|[A-Za-z0-9._-]+\.d)/[0-9]+[A-Za-z0-9._-]*")
...
# runner.sh also discovers tests recursively (`find "$d" -name '*.sh'`), so a nested
# `x.d/sub/10-y` is a name it can produce: carried, not resolved, and said so.
_TEST_REF = re.compile(r"(?:base|[A-Za-z0-9._-]+\.d)/[A-Za-z0-9._/-]+")
...
def _job_with_blocks(data: object) -> Iterable[tuple[str, dict]]:
    """Each job's `with:` mapping — the inputs it passes to the workflow it calls.

    Scoped to `jobs`, which is what keeps an input DECLARATION out of the results:
    qa-gate.yml describes `require_tests` in a `description: >-` under
    `on.workflow_call.inputs`, and a text scan reads that prose as a value.
    """
    ...
    for jid, job in sorted(jobs.items()):
        if isinstance(job, dict) and isinstance(job.get("with"), dict):
            yield str(jid), job["with"]


def _require_declarations(w: dict) -> Iterable[tuple[str, str]]:
    """The require-set declarations in one job's `with:`, as (where, value).

    Two doors, and they must agree: the `require_tests` input is the CI-side gate, and
    the `INSTANCE_TEST_REQUIRE_PASS` inside `extra_env` is the runner's own. They fail
    ...
    if isinstance(env, str):
        for line in env.splitlines():
            ...
            elif "${{" in line and not _ENV_ASSIGNMENT.match(line):
                # A line that is not `KEY=value` but carries an expression -- most
                # plainly `extra_env: ${{ matrix.extra_env }}`. It can expand to an
                # INSTANCE_TEST_REQUIRE_PASS line this check never sees, so it is named.
                # An expression INSIDE another key's value (comfyui's
                # PROVISIONING_COMFYUI_WORKFLOWS=...${{ github.sha }}...) cannot, and
                # stays quiet.
                yield "extra_env", line
    elif env is not None:
        yield "extra_env", _UNREADABLE


def _image_dir_for_template(repo: Path, template_dir: str) -> Path | None:
    """The image a QA template belongs to.

    An image OWNS the template nested under it, so the path prefix is exact and works
    at any depth -- including `derivatives/pytorch/derivatives/<name>`, where 16 of this
    repo's images live. The repo-root templates (`templates/base-qa`, `templates/
    pytorch-qa`) belong to an image somewhere else in the tree, and they SAY which one:
    `templates/pytorch-qa/template.yml` declares `image: vastai/pytorch`. Deriving it
    from the directory stem instead guessed, and guessed only in `derivatives/<stem>`
    and `external/<stem>` -- so a repo-root template for any nested image resolved to
    nothing and reported that image's own tests as missing.
    """
    if "/templates/" in template_dir:
        return repo / template_dir.split("/templates/")[0]
    if not template_dir.startswith("templates/"):
        return None
    import yaml  # lazy
    tpl = repo / template_dir / "template.yml"
    try:
        declared = (yaml.safe_load(tpl.read_text(encoding="utf-8", errors="replace"))
                    or {}).get("image")
    except Exception:
        return None
    if not isinstance(declared, str):
        return None
    stem = declared.rsplit("/", 1)[-1].split(":")[0]
    for img in discover(repo):
        if img.name == stem:
            return img.dir
    # The base image is discovered under the checkout's directory name, so a clone
    # named anything but `base-image` would not match `vastai/base-image` by stem.
    return repo
...
def check_workflow_required_tests_exist(repo: Path) -> Iterable[Finding]:
    ...
    for wf in sorted(wfdir.iterdir()):
        ...
        for jid, w in _job_with_blocks(data):
            ...
            # An image is claimed by the template_dir the job boots, so a workflow
            # copied between images is judged against the image it actually builds
            # rather than against whichever suite happens to exist somewhere.
            tdir = w.get("template_dir")
            tdir = tdir if isinstance(tdir, str) else ""
            if "${{" in tdir:
                yield Finding("L097", WARN, "-", rel,
                              f"job `{jid}` boots a template chosen at runtime "
                              f"(`{tdir[:40]}...`), so the image its required tests "
                              f"belong to cannot be read here and they are ungated "
                              f"(L097)")
                continue
            img = _image_dir_for_template(repo, tdir) if tdir else None
            ...
            for where, value in declarations:
                ...
                if "${{" in value:
                    yield Finding("L097", WARN, "-", rel,
                                  ...
                                  f"expression in `{where}` (`{value.strip()[:40]}...`), "
                                  f"which this check cannot read - the names in it are "
                                  f"ungated (L097)")
                    continue
                for tok in value.replace(",", " ").split():
                    if _TEST_NAME.fullmatch(tok):
                        names.add(tok)
                    elif _TEST_REF.fullmatch(tok):
                        ...
                        yield Finding("L097", WARN, "-", rel,
                                      f"job `{jid}` requires `{tok}` in `{where}`, "
                        ...
                        # runner.sh only discovers `base/` and `*.d/` suites and names
                        # each test by its path under them, so a token without that
                        # prefix can NEVER equal a test name. Not "cannot check": certain
                        # to fail, so an ERROR.
                        yield Finding("L097", ERROR, "-", rel,
                                      f"job `{jid}` requires `{tok}` in `{where}`, which "
                                      f"can never match a test: runner.sh names tests "
                                      f"`base/<name>` or `<suite>.d/<name>` - the gate "
                                      f"would fail on a rented GPU (L097)")
            ...
            for name in sorted(names):
                ...
                if suite is None:
                    ...
                    yield Finding("L097", ERROR, "-", rel,
                                  f"job `{jid}` requires `{name}`, but the image this "
                                  f"job builds ships no `{suite_name}/` suite (it "
                                  f"ships: {owned}) - a workflow copied between images "
                                  f"names the suite it came from (L097)")
                    continue
                # runner.sh compares test names with ==, so a prefix that passes here
                # still fails on a rented GPU with "missing from this image".
                if not any(f.name == f"{stem}.sh" for f in suite.iterdir()):
                    yield Finding("L097", ERROR, "-", rel,
                                  f"job `{jid}` requires `{name}`, which is not a file "
                                  f"in {suite.relative_to(repo)} - the gate would fail "
                                  f"on a rented GPU over something visible here (L097)")


# L098 — a serverless image owns its engine's log path.
_PROGRAM = re.compile(r"^\[program:([A-Za-z0-9._-]+)\]", re.M)
# Sidecars, not the engine: naming one of these in MODEL_LOG would point the worker at
# a log that never carries the model-loaded line.
_INFRA_PROGRAMS = {"ray", "model-ui", "caddy", "jupyter", "syncthing", "cron",
                   "instance_portal", "tunnel_manager", "pyworker", "sshd", "portal"}


# The serverless workers that READ `MODEL_LOG` from the environment. This is the whole
# scope of L098, and it is not the same set as "images that bake a BACKEND".
#
# Upstream, only `workers/openai/core.py` consults the environment:
#     model_log_file=os.environ.get("MODEL_LOG", defaults.model_log_file)
# `workers/{vllm,sglang,llama}` are the EngineDefaults feeding that core, so they read it.
# `workers/comfyui-json`, `workers/ace` and `workers/wan` are standalone and assign a
# module constant straight into WorkerConfig -- for those, baking the ENV changes nothing
# the worker does, and REQUIRING it would be worse than useless: it would certify a value
# nothing reads, so renaming the program while dutifully updating MODEL_LOG (as the rule
# would demand) leaves the worker tailing the old file and the gate green.
#
# comfyui shows the second cost: `MODEL_LOG` is not a private name there. Its own
# provisioning scripts use it with a different default
# (`MODEL_LOG="${MODEL_LOG:-/var/log/portal/comfyui.log}"`, plus the error traps in
# ~10 on-demand scripts), and the provisioner runs them with os.environ -- so baking it
...
def check_serverless_image_bakes_model_log(img: Image, repo: Path) -> Iterable[Finding]:
    ...
    programs = set()
    for conf in sorted(img.dir.glob("ROOT*/etc/supervisor/conf.d/*.conf")):
        try:
            programs.update(_PROGRAM.findall(conf.read_text(encoding="utf-8",
                                                            errors="replace")))
        except Exception:
            continue
    engines = sorted(p for p in programs if p not in _INFRA_PROGRAMS)

    # Fail CLOSED on ambiguity. With one engine program the right value is decidable;
    # with two, which one emits MODEL_LOAD_LOG_MSG is a property of the launch scripts,
    # and a rule that accepted either would bless the wrong one. Every image in scope
    ...
    if not model_log:
        yield Finding("L098", ERROR, img.name, "Dockerfile",
                      f"bakes BACKEND={backend} but no MODEL_LOG, so the worker falls "
                      f"back to a filename hardcoded in another repo and this image "
                      f"passes only while the two happen to agree. This image writes "
                      f"{expected} (L098)")
        return

    if model_log != expected:
        yield Finding("L098", ERROR, img.name, "Dockerfile",
                      f"MODEL_LOG={model_log} is not this image's engine log - its "
...
# L099 — a qa-gate caller's selection inputs come from a validated preflight output.
#
# The two qa-gate inputs that decide WHAT is rented and at WHAT PRICE. The rest (tag,
# label, retries, ...) shape the run but not the hardware a pass certifies.
_QA_SELECTION_INPUTS = ("set_filters", "max_price")
_RAW_DISPATCH_INPUT = re.compile(r"\$\{\{[^}]*\b(?:github\.event\.)?inputs\.[A-Za-z_]")


def check_qa_selection_is_validated(repo: Path) -> Iterable[Finding]:
    """L099 — no qa-gate caller passes a raw dispatch input to set_filters/max_price."""
    import yaml  # lazy
    wfdir = repo / ".github" / "workflows"
    if not wfdir.is_dir():
        return
    for wf in sorted(wfdir.iterdir()):
        ...
        if not isinstance(jobs, dict):
            continue
        for jid, job in sorted(jobs.items()):
            if not isinstance(job, dict) or "qa-gate.yml" not in str(job.get("uses", "")):
                continue
            w = job.get("with") if isinstance(job.get("with"), dict) else {}
            for key in _QA_SELECTION_INPUTS:
                val = w.get(key)
                if isinstance(val, str) and _RAW_DISPATCH_INPUT.search(val):
                    yield Finding("L099", ERROR, "-", rel,
                                  f"job `{jid}` passes `{key}: {val.strip()}` straight "
                                  f"from a dispatch input - route it through a preflight "
                                  f"step that validates it (custom tag only, compute_cap "
                                  f"only, bounded price; ADR 0047) and pass that step's "
                                  f"output (L099)")
...
def lint_image(img: Image, repo: Path, *, apply_exceptions: bool = True) -> list[Finding]:
    ...
    out.extend(check_serverless_image_bakes_model_log(img, repo))
