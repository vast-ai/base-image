...
RULES: list[tuple[str, str, str]] = [
    ...
    ("L093", ERROR, "A suite that greps its engine log for ERROR/CRITICAL must exclude the sentinel its OWN sibling test deliberately provokes. Every engine suite ships a `contract_check.py` defining `NO_SUCH_MODEL = \"__vast_contract_no_such_model__\"` and posts it to /v1/chat/completions on purpose — check_unknown_model asserts a nonexistent model is REFUSED rather than quietly substituted, so the engine is SUPPOSED to log that request as an error. The serving test in the same directory then greps the same log for ERROR/CRITICAL and fails the instance. It is not a first-pass problem, which is why it survived: discovery order runs `10-<engine>-serving.sh` before `12-<engine>-contract.sh`, so on a cold boot the sentinel is not in the log yet. It bites on the SECOND run — `runner.sh --manual` over SSH, which is exactly what the qa-fix loop does on a held instance — where run N's probe fails run N+1 and the reported failure names a model nobody asked for. Found 2026-09-07 on vLLM serving Qwen3.8-Flash-Next; the identical exposure was then latent in all three siblings (sglang, vllm-omni, llama-cpp), each of which defines the same sentinel and greps its own log without excluding it. Scoped to a `*.d` suite that has BOTH halves, and to the call whose label matches the suite stem — a sidecar log (`check_log_errors \"ray\"`) never sees the probe and must not be made to carry a meaningless exclusion. A suite with the sentinel but no label-matching call is also a finding: it means the naming convention the check reads has broken and the engine log is no longer identifiable"),
...
# the engine logging it is the ASSERTION SUCCEEDING, not a defect.
...
def _contract_probe_suites(repo: Path):
    """(suite_dir, engine_label, sentinel) for every *.d suite that provokes an engine error.

    The engine label is the suite stem: `vllm.d` -> `vllm`, `vllm-omni.d` -> `vllm-omni`.
    That is the same string the suite passes to check_log_errors for its own engine, and
    it is what separates the engine log from a sidecar's.
    """
    roots = [repo / "ROOT"]
    roots += sorted((repo / "derivatives").rglob("ROOT/opt/instance-tools/tests"))
    roots += sorted((repo / "external").rglob("ROOT/opt/instance-tools/tests"))
    ...
    for root in roots:
        ...
        for sub in sorted(tests.iterdir()):
            ...
            if not cc.is_file():
                continue
            m = _NO_SUCH_MODEL_DEF.search(cc.read_text(encoding="utf-8", errors="replace"))
            if m:
    ...
    for sub, engine, sentinel in _contract_probe_suites(repo):
        ...
        for sh in sorted(sub.glob("*.sh")):
            ...
            for lineno, raw in enumerate(body.splitlines(), 1):
                ...
                matched_any = True
                if sentinel in m.group(3):
                    continue
                yield Finding("L093", ERROR, "-", f"{sh.relative_to(repo)}:{lineno}",
                              f"this scans the {engine} log for ERROR/CRITICAL but does not "
                              f"exclude {sentinel}, which {sub.name}/contract_check.py posts "
                              f"to the engine ON PURPOSE — a re-run of the suite fails on its "
                              f"own probe from the previous run (L093)")
        if not matched_any:
            yield Finding("L093", ERROR, "-", str(sub.relative_to(repo)),
                          f"this suite defines the {sentinel} probe but has no "
