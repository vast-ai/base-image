...
- An app that rejects a request whose Origin differs from its own Host (Wan2GP since
  ...
  `CADDY_HEADER_UP_LOCALHOST` (ADR 0042, ADR 0043). Whether an app checks Origin is upstream
...
### An image whose app checks Origin declares its port for `CADDY_HEADER_UP_LOCALHOST` — ADR 0042/0043, **enforced by test_caddy_localhost_ports_sh.py**

Caddy forwards `Host: {upstream_hostport}` (always `localhost:<port>`) and the browser's
...
- An image whose app needs the rewrite ships `vast_boot.d/05-caddy-localhost-ports.sh`.
  The body is identical in every image; only the `caddy_localhost_ports=(...)` line
  differs. Today: wan2gp `${WAN2GP_PORT:-7860}`, ace-step `3000`, aio-studio
  `${WAN2GP_PORT:-17861} 3000`.
- The ports are INTERNAL ports (the third field of a `PORTAL_CONFIG` entry), and each must
  be the port the image actually serves that app on: the launcher's default, or, for the
  ACE Step UI, the port hardcoded upstream, which both Dockerfiles assert at build.
- The stage adds each port that is missing and keeps the template's entries. `true` means
  every port, so it is never appended to: that would turn it into a list and remove the
  rewrite from every other app. The stage parses the variable the way
  `caddy_config_manager.py` does, and the test pins both.
