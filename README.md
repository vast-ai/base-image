...
Remote calls require an auth token, and you need one whose value you know. `OPEN_BUTTON_TOKEN` is always auto-generated by the platform — you cannot set it (a supplied value is overwritten), and it can't be read from outside without SSH. So to authenticate remotely, either:

- **Set `WEB_PASSWORD` at launch** (recommended for automation): pass it as a template environment variable and use that value as the bearer token; or
- **Read the generated token over SSH**: `echo $OPEN_BUTTON_TOKEN` (it is exported to `/etc/environment`).
