...
- **Minor-version compatibility** ([docs](https://docs.nvidia.com/deploy/cuda-compatibility/)):
  ...
  - **PTX-JIT caveat (this is the one that catches people out):** minor-version
    compatibility covers the CUDA *runtime and libraries* but **not PTX JIT
    compilation**. The PTX→SASS JIT compiler lives in the *driver*, so if code is
    shipped as **PTX** generated by a toolkit *newer* than the host driver, the
    older driver can't JIT it — you get `CUDA_ERROR_UNSUPPORTED_PTX_VERSION (222)`
    or "the provided PTX was compiled with an unsupported toolchain", even though
    the runtime loaded fine and `driver_max_cuda` looked sufficient. This only
    bites when the host driver is *older* than the build toolkit (the reverse of
    the usual case here, where the host driver is newer). Fixes, in order: use a
    binary that ships **native cubins/SASS** for your arch (no JIT needed — most
    prebuilt wheels do for mainstream archs); or rely on **forward compatibility**
    (below) to get a newer driver JIT; or pick a build whose toolkit is `<=` the
    host driver. Newest GPUs are most exposed: a brand-new arch often has only PTX
    in older builds, so it must JIT — which is also why Blackwell needs a CUDA
    12.8+ build (see architecture, below).
